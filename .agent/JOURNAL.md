...
### DONE · 5.1-review · lp · 2026-08-19T14:19:41+00:00

**Review 5.1 @ 8af3787 — judge the commit, file findings, no edits**

clean, no findings — 8af3787 verified against SPECS.md 5.1, ARCHITECTURE.md §2.2 (Enemies row unchanged, host-only throughout), D-097. tools/enemy_ai_check.gd: 19 assertions, 0 failures (cone acquisition-gate, LOS ray acquisition-gate, retention-ignores-perception both directions, one-hop alert wakes only the in-range untargeted packmate and not the two-hop control, attack-slot cap holds a 3rd attacker then admits it once a slot frees). 2.10's tools/enemy_check.gd and tools/enemy_net_check.gd (real two-process ENet) both still failures=0, unmodified per diff. Full boot (--quit-after 20) 0 ERROR: lines. No bare autoload identifiers (get_node_or_null for NetTransport/PlayerNet throughout, matching existing pattern). State-machine dispatch confirms _engaged_attackers() is only ever evaluated from CHASE/IDLE, so its 'other enemies' count can't double-count self. DELEGATION.md Current state carries the new EnemyDef fields + Enemy.alert() API; F-155 filed for the unrelated pre-existing PlayerHealth crash it hit along the way.

Commit at time of writing: `272c6d0`

---

### DONE · 5.5-review · lm · 2026-08-19T14:25:21+00:00

**Review 5.5 @ 59917ed — judge the commit, file findings, no edits**

One real finding filed: F-225 (medium) -- Enemy.alert() bypasses Boss._acquire_target()'s override, so a boss pulled into a fight via a nearby ally's pack-alert never engages (no boss_engaged/health bar/phase movesets) until it takes its first hit; untested by boss_check.gd (alert_radius_m=0.0 only covers the boss's own outgoing alert). Everything else is sound: D-116's three calls verified against the diff, network authority correct (no new §2.2 row, phase/move_index on the existing REPLICATION_MODE_ALWAYS synchronizer), EventBus emits fire from replicated setters not host-only guards (D-107/108 pattern followed from the start), no bare autoload identifiers, typed GDScript throughout. Verified myself: tools/boss_check.gd 45/45 PASS failures=0; full boot --quit-after 20 zero ERROR:; regression suite (enemy_check/enemy_ai_check/enemy_net_check/entity_check/combat_feel_check) all failures=0 with no ERROR: lines. boss_check's own exit-time resource-leak ERROR is the pre-existing tracked F-193 item, confirmed not a new regression.

Notes along the way:
- boss_check.gd: 45/45 PASS, failures=0 (confirmed via agent godot run). Full boot --quit-after 20: 0 ERROR:. Regression suite (enemy_check/enemy_ai_check/enemy_net_check/entity_check/combat_feel_check) all failures=0, no ERROR: lines. boss_check's exit-time 'ERROR: 2 resources still in use'/'ObjectDB leaked' is the pre-existing, already-tracked F-193 leftover (deliberately left undeclared, non-gameplay) -- not a new regression, reproduced it and confirmed against FINDINGS.md before ruling it out. Filed F-225 (medium): Enemy.alert() (enemy.gd:386) sets _target_peer/_target_node directly, bypassing Boss._acquire_target()'s override (boss.gd:100) -- a Boss pulled into a fight via a nearby ally's pack-alert never calls _update_phase(), so it stays DORMANT_PHASE (no boss_engaged event, no health bar, EnemyDef fallback attack instead of its BossPhaseDef moves) until it takes its first hit. boss_check.gd's alert_radius_m=0.0 on its own synthetic BossDef only suppresses the boss's OUTGOING alert, so this incoming path is untested. Everything else checks out: D-116's three calls (zero enemy.gd edits, arena as data-flag+leash not geometry, no new §2.2 row) hold up against the actual diff; EventBus emits correctly hang off phase's/state's replicated setters (D-107/D-108 pattern, not host-only guards); no bare autoload identifiers in code (EVENT_BUS preload const throughout); DELEGATION.md's public API list and DECISIONS.md D-116 both match the shipped code.

Commit at time of writing: `0393adf`

---

### DONE · 5.9-review · lp · 2026-08-19T14:26:11+00:00

**Review 5.9 @ d8dc751 — judge the commit, file findings, no edits**

Clean implementation, one finding filed. d8dc751 verified against SPECS.md 5.9, ARCHITECTURE.md §2.2 (wave director row, unchanged, host-only), D-113. wave_director_check.gd: 19/19 PASS, only the declared 'material is null' pattern in ERROR output. Regression suite (wave_spawner_check.gd, cycle_check.gd, cycle_modifier_check.gd) all failures=0, full boot 0 stray ERROR:. Cycle-count multiplier formula and composition-weight math both checked by hand against the code, match spec exactly. No bare autoload identifiers, typed GDScript throughout, seeded RNG per subsystem, no new RPC (matches authority table). Filed F-226 (medium): current_cycle()/cycle_count_multiplier() claim 'readable on any peer' but the client-side cache never updates (CycleService only emits EventBus.cycle_advanced when it owns the Cycle) -- a trap for 5.9's own stated future HUD/debug consumer, not exploited by anything shipped today. Note: F-226 landed inside lm's concurrent 8816c40 (Review 5.5) commit, not a commit of mine -- docs/FINDINGS.md was already clean in my working tree by the time I went to commit, the F-149/nettle12 sweep-by-bare-commit hazard AGENTS.md already documents, not a new problem. Content is correct and in the repo either way.

Notes along the way:
- Verified d8dc751 against SPECS.md 5.9, ARCHITECTURE.md §2.2 (wave director row, host-authoritative, no new RPC), D-113. wave_director_check.gd: 19/19 pass, only declared error pattern seen. Regression: wave_spawner_check.gd, cycle_check.gd, cycle_modifier_check.gd all failures=0; full boot 0 stray ERROR. Composition weighting math and cycle_count_multiplier cap verified by hand against the formula. Filed F-226 (medium): WaveSpawner.current_cycle()/cycle_count_multiplier() are documented 'readable on any peer' but CycleService._announce() only calls EVENT_BUS.emit_cycle_advanced() when _owns_cycle() is true (host/solo), so a real client's local cache never leaves its Cycle-1 default -- a trap for the future HUD/debug consumer SPECS.md 5.9 names as this API's purpose. Not exploited by anything shipped in 5.9 itself (host_start_wave is host-only).

Files: `docs/FINDINGS.md`

Commit at time of writing: `8816c40`

---

### DONE · 6.7-review · lm · 2026-08-19T14:27:47+00:00

**Review 6.7 @ 97f7d47 — judge the commit, file findings, no edits**
...
### DONE · 3.17-review · lp · 2026-08-19T14:59:21+00:00

**Review 3.17 @ ad916c2 — judge the commit, file findings, no edits**

Sound overall, one medium finding (F-230): FunctionRunner.effective_scope() classifies a function's routing scope by each line's DECLARED max scope (CommandService.scope_of()) rather than that line's actual D-086 invocation scope, so a function whose only content is a LOCAL-form dynamic-scope line (time query, a bare rule read, etc.) is wrongly forced to HOST and refused to non-ops -- reproduced live (direct 'time query' as non-op: ok=true; same line wrapped as 'function ...': ok=false, not-op). No shipped content hits it (night_siege.mcmd/dev_scenario.mcmd use only fixed-scope commands) and it fails closed, never open, so not high. Everything else traced clean: peer attribution correct end to end (net_submit_command builds ctx from the real sender_id, _cmd_function threads ctx unchanged via ctx.duplicate() so a non-host op's function runs as THEM, not host -- the exact class of bug F-228 found in 3.16 does not recur here). Hook signal bindings (_HOOK_EVENTS) checked against real signal signatures: DayNight.night_started/day_started (no args) and EnemyWorld.enemy_died(enemy_id, instigator_peer_id, position) both match their handlers' arity exactly. Recursion cap (4) consistent between routing (FunctionRunner.effective_scope) and runtime (ctx._fn_depth in _cmd_function). Autoload order (CommandService before Registry/DayNight/EnemyWorld) confirmed in project.godot, call_deferred wiring works (function_check.gd's live dusk-crossing test passes). Rule 1 (bare autoload names) respected via get_node_or_null everywhere new. F-016 preload respected (FUNCTION_RUNNER/HOOK_DEF preloaded, one-directional, no cycle). D-094/F-154 companion docs (1210c43) consistent with the code. Verified: agent godot --script tools/function_check.gd (0 failures, 21 assertions), tools/command_catalog_check.gd (0 failures, 45 commands incl. function), agent godot --script tools/run_commands.gd -- --file content/functions/dev_scenario.mcmd --json (0 failures) and a scratch expect-fail file (correctly caught a deliberately-wrong expectation, failures=1), full boot agent godot --quit-after 20 (0 ERROR: lines).

Notes along the way:
- Filed F-230 (medium): FunctionRunner.effective_scope() uses scope_of() (declared max) instead of per-line invocation scope for D-086 dynamic-scope commands, so a function containing only a LOCAL-form dynamic-scope line (e.g. 'time query') is misclassified HOST and wrongly refused to non-ops. Reproduced live via scratch script. No shipped content hits it. Everything else traced clean.
...
### DONE · F-261-review · lm · 2026-08-20T05:29:07+00:00

**Review F-261 @ c05c066 — review and fix what you find**

VERDICT: F-261 is sound — no defect in the change, nothing reverted or rewritten. Verified independently rather than trusting the commit message: agent godot --script tools/worldgen_noise_reuse_check.gd -> WORLDGEN_NOISE_REUSE failures=0, 0 ERROR: lines, all four groups green (equivalence 361 samples x 5 seeds for moisture/continent/biome_at/terrain_amplitudes from_set vs bare; adoption identical and by reference; layout identity — POI/biome/amplitude grids hash to the pre-fix GOLDEN_* constants on all 5 seeds; poi determinism; speedup 1.45x vs a 1.3x floor). Neighbours re-run: poi_check 0, biome_check 0, resource_scatter_check 0, noise_reuse_check PASS 0 failures, terrain_check 0 failures, check_determinism terrain_hash c20eed19b44270a1 (matches F-241's recorded value). Read the diff line by line: poi_map's lazy biome resolution is exactly equivalent on all three relax levels, _slope_at's passed-in centre matches the caller's default-amplitude height, continent()/continent_from_set() build the same four fields as the old _warp_point path, and terrain_map_render's restructure yields the same h and the same tint id. Rule 1 clean (every cross-script reference is a preload const). ARCHITECTURE §2.2's 'POI placement' row is quoted accurately. Finding is under ## Resolved; state.json agrees; SPECS/DELEGATION/DECISIONS all describe what shipped. THREE RECORD FIXES made under this claim: (1) the '22 million FastNoiseLite fields at the default 1024 px' figure was wrong in four places — terrain_map_render.gd's own comment, FINDINGS, SPECS and DELEGATION — the tool's default is 600 px, so it is 22 constructions/pixel = ~7.9M at 600 and 23M at --size 1024; corrected everywhere and the tool re-run (TERRAIN_MAP ... (ok)). (2) docs/NEXT.md was materially false: 120/165 tasks (now 126/165 with per-milestone counts recomputed from state.json), 'Findings: 7 open, and not one of them is code you can sit down and write' (actually 23 open, of which nine are ordinary code work), and a boot content line reading 3 POIs/1 cycle modifier/1 unlock/2 enemies against today's 6/7/7/5. (3) DELEGATION's F-261 API block now carries the caveat below. FILED F-274 (worldgen, medium): BiomeMap.terrain_amplitudes()/terrain_amplitudes_from_set()/amplitudes_for() have NO shipped caller — chunk_mesher.gd:150, poi_map.gd:153 and resource_scatter.gd:120 all take height()'s 1.0/1.0 defaults, so every biome's authored detail_amplitude/ridge_amplitude is inert and only tools/terrain_map_render.gd applies them. D-144 says 'BiomeMap.terrain_amplitudes() is the seam that hands a point's pair to the heightmap'; the seam was built and nothing crosses it, which is the F-212 shape. Consequence: the audit PNGs are the only place that terrain exists, and F-271's premise ('height() DEPENDS on which biome a point is in') is not true today — wiring this is what makes F-271 live, so they want fixing together. Not F-261's doing: F-261 preserved the behaviour exactly and its amplitudes_for() is the seam's public half; found because this review traced every caller of the amplitude API and the shipped list came back empty.

Notes along the way:
- Verified F-261 myself: worldgen_noise_reuse_check failures=0 (equivalence 361x5, adoption, layout-identity goldens, poi determinism, 1.45x speedup), 0 ERROR: lines. Neighbours re-run green: poi_check 0, biome_check 0, resource_scatter_check 0, noise_reuse_check PASS 0, terrain_check 0, check_determinism terrain_hash c20eed19b44270a1. No defect found in the change itself.
...
### DONE · F-287 · lp · 2026-08-20T10:57:18+00:00

**Procedural reseed accumulates EnvironmentVfx emitter sites from every previous island**

EnvironmentVfx now retires an emitter site with the prop it came from, so an in-place reseed REPLACES its site set instead of appending to it. Each site records its source node's instance id (_site_sources, parallel to _sites, source = _emitter_host so it is the prop not a mesh part); _prune_dead_sites() drops any whose source is invalid-or-out-of-tree and recomputes emitter_site_count/fire_source_count from the survivors. Runs from two places because one is not enough: a deferred _rediscover_world() on EventBus.run_restarted (the shipped boundary) AND the existing quarter-second budget tick (which is the only thing that catches rebuild_for_seed() called directly, a scatter chunk streaming out, or a prop freed mid-run). NOT a clear on run_restarted — D-170: the authored map fires the same signal and rebuilds nothing, its props all carry VFX_META, so a clear-and-re-walk would skip all of them and take Hollowmere's 5 fires, 101 crystals and 161 spore sites dark with no error. The new check plants a survivor prop outside the rebuilt subtree so that mistake fails a check instead of shipping. Proves it: .agent/bin/agent godot --script tools/environment_vfx_reseed_check.gd -> ENVIRONMENT_VFX_RESEED_CHECK failures=0, RESTART before=22 after=17 shared=0 sway_assets=2. Pre-fix, same check: failures=3 with 22->38->52 sites and all 21 old ones surviving. Siblings green and unchanged: ENVIRONMENT_VFX_CHECK foliage=8103 failures=0, ENVIRONMENT_VFX_HOLLOWMERE_CHECK failures=0, RUN_RESEED_CHECK failures=0. Spec written (none existed), D-170, DELEGATION Current state, F-303 filed for the one counter left cumulative.

Notes along the way:
- Design call: the fix is a SOURCE-TRACKED prune, not a blind clear on run_restarted. A blind _sites.clear() would permanently erase the AUTHORED map's emitter sites — Hollowmere fires run_restarted too (F-243) but rebuilds no geometry, and its surviving nodes all carry VFX_META so the re-walk skips them and never re-registers. Each site now records the instance id of the node it came from; the prune drops only sites whose source is freed or out of the tree (remove_child is synchronous, so ProceduralWorld._teardown_derived's nodes read as out-of-tree immediately, before queue_free lands at frame end).
- Pre-fix proof: with the subscription and the periodic prune commented out (copy kept in scratchpad, not git stash — a stash would touch other lanes' uncommitted work), tools/environment_vfx_reseed_check.gd -> failures=3 and the exact symptom the finding describes: 22 sites before the restart, 38 after, all 21 of the ended island's sites still registered, and a third island took it to 52. Fixed: failures=0, before=22 after=17 shared=0. The check also had to call ChunkStreamer.set_anchors() itself — with build_player=false nothing anchors the streamer, no chunk reaches LOD0-with-collision, ResourceScatterField builds no holder, and the island registers ZERO emitter sites however long it is left; the first run of this check measured before=2, both of them its own planted props.
- Sweep of the class (autoload cache of scene-derived state whose only invalidation is the current-scene instance id): three siblings share the shape — crafting_service.gd (_station_scene_id, already F-286, another lane's), harvest_world.gd (_observed_scene_id — NOT affected: it is a 'has the main scene been assigned yet' edge trigger, and wired_harvestables() re-derives from the live group every call), graphics_quality.gd (_applied_scene_id — not affected either; the nodes it touches, the sun and WorldEnvironment, live on the level and are not what rebuild_for_seed() frees, and DrawPolicy.apply() reads the preset at creation time so a new island's props are born at the right draw distance). The four node_added services (wellspring, chest_placement, extraction, harvest) hold no position arrays at all — they build content on markers and re-derive from groups, so an in-place rebuild is their ordinary path. EnvironmentVfx was the only one accumulating.
- Nearly filed a finding against graphics_quality.gd's _authored_lights/_authored_environments (keyed by instance id, never pruned, and its own comment reasons 'stale entries are only a few bytes'). Checked the premise before filing rather than after: Godot 4 does NOT hand a freed object's id to a new one — ObjectID packs a validator that changes when the slot is reused. Probed headless, 500 freed meshes vs 5,000 fresh allocations, 0 collisions. That file's reasoning is correct as written, and the same correction applies to my own _prune_dressed_meshes, whose first comment claimed recycling as its justification. It stays for the census reason (sway_asset_count climbing by a whole island per restart), not a correctness one.
...
### DONE · F-271-review · lp · 2026-08-20T11:21:16+00:00

**Review F-271 @ cffbbdd — review and fix what you find**

SOUND — no defect found, nothing filed. Re-ran both checks myself at HEAD through agent godot: RESOURCE_SCATTER_CHECK failures=0 (30/30 PASS, incl. 'every placed point's world position actually resolves to its table's biome') and WORLDGEN_NOISE_REUSE failures=0 (all four goldens green on all five seeds, shared-set speedup 1.57x). No ERROR: lines in either run; the only non-PASS output was the known EnemyWorld 'navmesh baked 0 polygons' WARNING, which is a warning and pre-existing. Judged against the spec: the fix does what the F-271 SPECS block contracts — biome via BiomeMap.biome_at_from_set(), gate moved above the surface sample, rng stream order unchanged, check re-derived independently through continent()+moisture()+assign() rather than mirroring biome_at(). Authority: correct, ResourceScatter is pure and rides ARCHITECTURE 2.2's chunk-streaming row; no client mutates shared state. Rule 1: both tools reach Registry via root.get_node_or_null(^"Registry"), no bare autoload identifiers. Determinism: no randi(), per-point seeded RNG untouched. I verified the D-163 sweep claim independently rather than trusting it — the only remaining BiomeMap.assign() callers repo-wide are biome_map.gd's two internals and biome_check.gd's unit tests of the primitive. Also checked the one hazard the continental reinterpretation could have opened and the task did not mention: assign()'s fallback is a discontinuity, so a point above every band's height_max would resolve to shore on a mountain top. Shipped bands cover -100..100 continental metres and IslandHeightmap.MAX_HEIGHT is (1+LAND_BIAS)*26 + RIDGE_WEIGHT, far below that, so the fallback is unreachable. Bookkeeping fixed under this claim (three files, no code): (1) docs/SPECS.md's F-271 block described the INTENT, not HEAD — F-274 landed one commit later and changed two of its sentences, so I added a 'Superseded in part by F-274' section saying position.y now comes from BiomeMap.surface_from_set() with a per-chunk TerrainTable, and that the three GOLDEN_* that F-271 correctly required to be UNCHANGED were re-captured by F-274, leaving GOLDEN_BIOME as the invariant that has survived F-261/F-271/F-274 untouched. This is the exact F-212 failure mode the review order names. (2) docs/NEXT.md said 23 open findings and offered a 'pick up today' list of nine, four of which (F-259, F-268, F-271, F-274) were already resolved — re-counted to 34 from FINDINGS' Open section, named the 14 the day's lanes closed, and replaced the stale list with five routing clusters covering all 26 that are real work. (3) ARCHITECTURE 2.2's chunk-streaming row named IslandHeightmap/ChunkMesher but not ResourceScatter, which the spec explicitly declares rides that row — added it, with the reason the divergence was invisible at runtime (no authoritative answer to contradict). F-271's finding section is already correctly under '## Resolved' and its board entry is accurate.

Notes along the way:
- Review verdict: F-271's code fix is SOUND. resource_scatter_check failures=0 and worldgen_noise_reuse failures=0 at HEAD, both re-run by me, no ERROR: lines. Swept every assign()/biome_at caller myself: the only remaining BiomeMap.assign() callers are biome_map.gd's own two internals and tools/biome_check.gd's unit tests of the primitive, so D-163's rule holds repo-wide. Checked the fallback hazard the continental reinterpretation could have opened — shore/grassland/forest cover -100..100 continental metres and IslandHeightmap.MAX_HEIGHT is (1+LAND_BIAS)*26 + RIDGE_WEIGHT, far under 100, so assign()'s discontinuous fallback is unreachable and no peak silently resolves to shore. Filing nothing.

Files: `docs/NEXT.md`, `docs/SPECS.md`, `docs/ARCHITECTURE.md`

Commit at time of writing: `89b51b6`

---

### DONE · F-274-review · lp · 2026-08-20T11:35:30+00:00

**Review F-274 @ 8754844 — review and fix what you find**

SOUND — nothing filed against the terrain work. biome_terrain_check failures=0, 0 ERROR lines (5 sections: authored pair reached, table moves 957-1211 of 1217 chunk vertices across 4 seeds, mesh/both samplers/PoiMap agree bit-for-bit, worst amplitude step 0.0092 vs 0.05 limit, build_mesh still requires the table). Neighbours: worldgen_noise_reuse 0 with GOLDEN_BIOME unchanged; check_determinism terrain_hash c20eed19b44270a1 unchanged + new biome_surface 1e6501a6940d5bd0; chunk_stream_check --windowed 0 functional failures at the recorded 12.4405 m; noise_reuse/terrain_look/terrain_check/poi_check/biome_check/resource_scatter/procedural_world/world_contract all 0. Fixed two defects under this claim: nav_baker.gd bind()'s 'streamer.get() as Array' aborts the function on a Nil rather than yielding null, so its own null guard was dead and bind() returned false with the navmesh silently never baking (pre- and post-fix both proven with a scratch probe); and DELEGATION's F-274 API block listed continent_from_shape/height_from_shape with a world_seed neither takes and omitted Shape.channel, the cache the same entry warns against removing. Verified after: nav_bake_check unchanged at its 4 pre-existing F-285/F-292 failures, procedural_world_check 0, biome_terrain_check 0.

Notes along the way:
- Verdict: F-274 is sound. Ran its own check and every neighbour the spec names: biome_terrain_check failures=0 / 0 ERROR lines, worldgen_noise_reuse_check 0 (GOLDEN_BIOME unchanged), check_determinism terrain_hash c20eed19b44270a1 unchanged + biome_surface 1e6501a6940d5bd0, chunk_stream_check --windowed 0 functional failures with the recorded 12.4405 m divergence, noise_reuse/terrain_look/terrain_check/poi_check/biome_check/resource_scatter_check/procedural_world_check/world_contract_check all 0. Nothing filed against the terrain work itself.
- GDScript trap, fixed in world/chunk/nav_baker.gd:bind(). `x as Array` on the Nil that Object.get() returns for a MISSING property is not null — it is a hard 'Invalid cast' runtime error that ABORTS the enclosing function. F-274 wrote 'biome_defs = streamer.get(&biome_defs) as Array' followed by 'if biome_defs == null', so the guard could never run; instead bind() aborted at that line and returned false, which ProceduralWorld._build_nav() ignores, so the streamer signals never connect and the navmesh silently never bakes. Proven both ways with a scratch probe: pre-fix 'SCRIPT ERROR: Invalid cast' + bind() false; post-fix ('var table: Variant = streamer.get(...); biome_defs = table if table is Array else []') bind() true, empty-table fallback, _ensure_map() reached. Unreachable today (the only shipped caller passes a real ChunkStreamer, and nav_bake_check passes null, which the outer 'if streamer != null' guards) — sibling of SPECS standing rule 5, which covers 'as Array[T]' but not this.
- Record fix (F-212 class), docs/DELEGATION.md F-274 entry: the 'API the next task builds against' block described signatures that do not exist — continent_from_shape(shape, world_seed) and height_from_shape(x, z, shape, set, world_seed, ...) both list a world_seed the shipped functions deliberately do not take, and the Shape field list omitted 'channel', the cached river ceiling that the SAME entry's cost note tells you not to remove (it is the 10.697 -> 8.077 ms/chunk saving). Corrected to the shipped signatures with a line saying why neither takes a seed. Everything else in the record checked out: F-274 is under '## Resolved' in FINDINGS, state.json/BOARD agree, NEXT.md was already re-counted by F-271's review, ARCHITECTURE 2.2's chunk-streaming row already names BiomeMap, D-165 matches the code.
- Two things I deliberately did NOT file, having checked them rather than assumed them. (1) nav_bake_check's 4 failures at HEAD are not F-274's doing: a scratch probe re-ran _locate_walkable_terrain's exact scan against the shaped table AND against F-274's own biome-blind control table (make_terrain_table([])) — both return NONE, so the island has no chunk boundary with gentle land on all four probes either way. F-285/F-292 stand as written. (2) D-165's 'the shipped content has no coverage gap, so the (1,1) fallback discontinuity is unreachable' is true and now measured: over 9 seeds x 40,401 points, carved continent spans -6.21..32.91 m, comfortably inside shore's -100..4 and grassland/forest's 4..100, and blend_amplitudes() returned the (1,1) fallback at 0 of 363,609 points.
...
### DONE · F-307 · lp · 2026-08-20T21:02:33+00:00

**A non-host peer whose host leaves is soft-locked on F-243's terminal overlay — the restart button never re-reads its own host check, and no menu can open over it**

A terminal run-summary overlay whose session ends is no longer a dead end. Both ui/hud/defeat_hud.gd and ui/hud/extraction_hud.gd now subscribe to NetSession.session_ended and carry a _session_over flag scoped to the SHOWING (not the process — transport state cannot tell a solo host from an orphaned client, both answer _is_host_or_solo() true). On session end the control becomes an enabled FOCUS_ALL 'Leave to Menu' opening MainMenu, the overlay leaves blocks_gameplay_input (without that half, D-032's interlock still refuses every menu and the screen still has no way out), and focus re-grabs per F-275. Deliberately NOT a restart — D-185 has the reasoning. Proven by a new phase 3 in tools/terminal_focus_check.gd: two more child processes, one per overlay, each recording the before-state (disabled waiting label, in the blocking group, MainMenu.set_open correctly REFUSED), then the driver ends the session and the child waits on session_ended itself before asserting the flip and tapping a real InputEventJoypadButton to reach MainMenu.is_open(). The two children end the session by different paths so both EndReasons are covered: defeat via the shipped NetTransport.leave() (CONNECTION_LOST after the ~19s rejoin ladder), extraction via NetSession.end_session() (HOST_CLOSED). Command: .agent/bin/agent godot --script tools/terminal_focus_check.gd -> TERMINAL_FOCUS_CHECK failures=0, 54 PASS, exit 0; the only ERROR lines are the 5 declared rejoin-ladder ones from the shipped ungraceful path. Pre-fix, that same phase 3 gave failures=10 (five per overlay). Spec written at docs/SPECS.md F-307 (none existed). Swept: _is_host_or_solo() exists in exactly those two files; the mandatory-panel class is exactly three files and the third, ui/attunement/attunement_ui.gd, has the same bug -> F-321. Also filed F-322: nothing shipped calls NetSession.end_session(), which is why an ordinary host quit costs clients 19s and reports 'connection lost'.
...
- Two ID collisions with concurrent lanes, both caught before ship — worth knowing that reserving a number early is worthless here. F-320 was taken by reed31c598 while I was writing the spec, so my two findings are F-321/F-322; D-184 was taken by quill5fa5c7 (4.18 terrain) while I was BLOCKED POLLING for the docs/DECISIONS.md claim, so my decision is D-185. Both required a sed pass across code comments, check assertion strings and three docs. Take the next number only once you actually hold the claim on the file, and re-derive it AFTER the claim succeeds, never before.
...
### DONE · F-410 · emberd89a44 · 2026-08-21T05:48:24+00:00

**Three visual defects remain in the noon frame after the grade fix: pale grey trunks, orange canopy facets, and a flat grey horizon**

Reset the shared daytime look to a neutral Valheim-directed baseline: controller-owned procedural sky, softer sun/fill/AO/fog, green-only birch/crooked crowns, and fixed-seed production renders. State checks all pass; ready for Sequoyah's visual playtest.

Notes along the way:
- Sequoyah clarified that 7cf1225 is diagnostic context, not an approved look. Re-establish the daylight target from neutral same-run renders; preserve no value without visual proof. GLB JSON proves pine/crooked bark materials contain the intended brown palette values and leaf_gold is deliberately exported, so F-092's old cache mismatch is not the live cause.
- Same-run noon probe: turbidity 20 visibly changes the frame, proving grade_probe's sky override path. Reasonable PhysicalSkyMaterial settings all retain the achromatic horizon lobe; a ProceduralSkyMaterial gradient removes it immediately. Neutral candidate (sun 1.15, ambient 0.48, exposure 0.92, white 1.5, saturation/contrast 1.0, SSAO radius 1.25/intensity 1.4/light affect 0.1) restores brown bark and midtones. Pine GLB and Godot import both carry correct brown albedos, so the 'grey pine' hypothesis is false; pale trunks in seed 4242 are intentionally birch. Orange canopy facets are explicit leaf_gold surfaces in birch/crooked generators.
- Root cause and resolution: F-092's material-cache mismatch is not present. Imported pine/crooked bark is brown; the tall pale trunks in the fixed-seed frame are intentionally birches (MIRE_WoodBirch 0.867/0.875/0.820). Orange canopy facets were explicit leaf_gold assignments in birch/crooked builders and are now removed from the standing summer palette. The grey horizon survived every PhysicalSky scattering/fog isolation; the controller now installs one ProceduralSkyMaterial gradient for every level. Noon reset is neutral grade (contrast/saturation 1.0), sun 1.15, ambient 0.48/78% sky, ACES white 1.5/exposure .92, restrained AO 1.25m/1.4/10% direct.
- Verification: grade_check 0 failures across procedural_island+hollowmere; atmosphere_night_check 0; graphics_quality_check 0. Godot import inspection of regenerated birch/crooked A shows brown bark or intentional pale birch plus green Leaf/LeafLight and no LeafGold. Fixed seed 4242 production renders recorded noon/golden/night; noon clips 0.01% of pixels and the flat grey horizon band is absent.
...
### DONE · F-321 · onyxbe8065 · 2026-08-22T05:57:23+00:00

**`AttunementUI` is the third mandatory panel and it still has F-307's soft-lock — it closes only on an accepted pick, so an orphaned client can never leave it**

Completed.

Files: `ui/attunement/attunement_ui.gd`, `tools/terminal_focus_check.gd`

Commit at time of writing: `983f3012`

---

### DONE · F-516 · onyxbe8065 · 2026-08-22T05:57:23+00:00

**Nothing pre-warms shipped materials, so every player pays shader/pipeline compilation as hitches during their first minutes of play**

Completed.

Files: `autoload/material_warmer.gd`, `tools/material_warm_check.gd`

Commit at time of writing: `983f3012`

---

### DONE · F-316 · cinder9818da · 2026-08-22T06:07:46+00:00

**docs/SPECS.md carries two different F-226 blocks under one heading, and nothing checks SPECS for duplicate headings**

merged the duplicate SPECS F-226 block; decision_ref_check now detects duplicate SPECS headings

Files: `tools/decision_ref_check.py`

Commit at time of writing: `47710430`

---

### HANDOFF · F-454 · onyxbe8065 · 2026-08-22T06:13:58+00:00

**The felt performance problem is chunk streaming, not rendering: traversal drops the 1% low from 81 fps to 13 fps, and no graphics preset touches it**

Measured, not fixed. tools/traversal_profile.gd run windowed (agent godot --windowed drops the injected --headless, F-077 — the finding's claim that this needs a human is wrong). 7886 frames: ChunkStreamer is only 2.9% of hitch time and reads 0.00ms on 19 of the 20 worst frames, so the streamer is NOT the cause. Hitch frames are the frames that add nodes: 99 per hitch frame vs 5 per quiet frame, 44049 over a 45s walk, with the scatter group queue 1100-1800 deep. Look at the per-chunk instantiate/enter-tree path in world/gen/resource_scatter_field.gd's drain, not the streamer's issuing loop. F-547 filed for one concrete contributor (EnvironmentVfx per-node deferred work plus 1821 backtraced push_warnings). Full numbers are in the F-454 entry in docs/FINDINGS.md. No code changed; no files claimed.

Commit at time of writing: `8ad6157b`

---

### DONE · F-306 · cinder9818da · 2026-08-22T06:30:37+00:00

**docs/NEXT.md is hand-maintained but summarises a board that moves hourly, so it goes stale within hours — corrected twice in one night by two different reviewers**

NEXT.md's counts are generated by tools/next_gen.py and checked for drift
...
### REOPEN · F-541 · nettled7199c · 2026-08-22T06:48:16+00:00

**Chest tiers: only three loot tiers are actually placeable and only three chest silhouettes exist**

Reopened (was marked done 2026-08-22 by nettled7199c).

D-215 was never written into docs/DECISIONS.md - the file was claimed at close-out time

Commit at time of writing: `670ee04a`

---

### DONE · F-541 · nettled7199c · 2026-08-22T06:49:05+00:00

**Chest tiers: only three loot tiers are actually placeable and only three chest silhouettes exist**

D-215 written into docs/DECISIONS.md and pushed (8c5a5bdc); scratchpad draft deleted. Docs debt from the original ship is closed. Remaining: docs/DELEGATION.md Current state entry for the ladder API - still claimed by larch543bba under F-543, watcher re-armed.
...
### DONE · F-552 · coil9c136f · 2026-08-22T07:00:19+00:00

**Shared worktree has 178 uncommitted artifacts**

Recovered 172 intentional outputs and removed six orphan scratch UID sidecars; JSON/JSONL and whitespace validation passed.
