...
## A-011 — verifies the gatherables kit as GODOT sees it, not as Blender reported it.
##
##   .agent/bin/agent godot --headless --script tools/gatherables_check.gd
##
## The build-time contract in `tools/blender/build_gatherable_plants.py` already fails on size,
## footprint, ground contact, triangle budget, materials and the state-pair anchor. This check
## exists because none of that is evidence about the shipped GLB: it measures the Blender scene,
## and every batch before this one has had at least one defect that only appeared after the export
## and re-import (F-094, F-108, and this batch's own 53 mm-underground resin node, which Blender
## reported as perfectly grounded because the measurement went through the object's world matrix
## while the exporter wrote the rotation out as a node transform).
##
## Vertices, never `Transform3D * AABB` (F-108/F-094). Every `cone`/`tapered_between` primitive in
## this kit is a rotated object, and an AABB pushed through a rotation inflates — that construction
## reported seven of A-009's fifteen exports oversized when nothing was wrong with them.

const CATALOG_PATH := "res://assets/gatherables/catalog.json"
const EXPORT_DIR := "res://assets/gatherables/exports/"
...
func _initialize() -> void:
	print("=== MIRE A-011 — gatherable plants and deposits ===")
