...
def main() -> None:
    ...
    # F-422: the trunk generator itself lives in `mire_art` now. Both kits had
    # grown their own copy of the frustum-stack version, so both carried all four
    # of its defects and both would have had to be fixed twice. Every dimension
    # it varies is an explicit argument, so sharing it reshapes nothing — the
    # caution in ASSET_TRACKER.md about not sharing geometry helpers is about
    # primitives with implicit DEFAULTS (vertex counts, end tapers), and this has
    # none that a caller does not pass.
    tree_trunk = trunk_tube
