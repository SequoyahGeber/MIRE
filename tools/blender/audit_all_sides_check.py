"""Regression guard for F-109: `audit_all_sides.py`'s inside-out test must not
judge an open sheet by the same divergence-theorem sum that only means
something for a closed shell.

Run:
    /Applications/Blender.app/Contents/MacOS/Blender --background --python \
        tools/blender/audit_all_sides_check.py

The bug this guards: `geometry_report()` used to run every mesh through
`sum (n . c) * area` and flag a negative result as "inside out" -- correct
only for a closed shell, where the divergence theorem gives an enclosed
volume. For an open sheet the same sum has no enclosed volume to measure and
is dominated by where the sheet sits relative to the world origin instead: a
bottom-strake patch board with a correct downward normal and a positive z
centre scored negative and read as a defect. A-009's finished, verified hull
still reported 94 such false positives under the old test. The fix --
`is_closed_shell()` -- welds vertices by position (these meshes are unwelded
face soup, which defeats bmesh's own `edge.is_manifold`) and only trusts the
volume sign when every welded edge borders exactly two faces; an open sheet is
now reported separately as `open_surface_objects` instead of being judged at
all.
...
from audit_all_sides import bounds, geometry_report, is_closed_shell  # noqa: E402
