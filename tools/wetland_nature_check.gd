...
## A-015 — verifies the wetland nature kit as GODOT sees it, not as Blender reported it.
##
##   .agent/bin/agent godot --headless --script tools/wetland_nature_check.gd
##
## Same instrument as `tools/gatherables_check.gd` (A-011) pointed at a different catalog, and it
## exists for the same reason: the build-time contract in `tools/blender/build_wetland_nature.py`
...
const CATALOG_PATH := "res://assets/wetland_nature/catalog.json"
const EXPORT_DIR := "res://assets/wetland_nature/exports/"
...
func _initialize() -> void:
	print("=== MIRE A-015 — wetland nature ===")
