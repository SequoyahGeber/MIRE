...
## A-016a — verifies the terrain accents kit as GODOT sees it, not as Blender reported it.
##
##   .agent/bin/agent godot --headless --script tools/terrain_accents_check.gd
##
## Same instrument as `tools/gatherables_check.gd` (A-011) pointed at a different catalog, and it
## exists for the same reason: the build-time contract in `tools/blender/build_terrain_accents.py`
...
const CATALOG_PATH := "res://assets/terrain_accents/catalog.json"
const EXPORT_DIR := "res://assets/terrain_accents/exports/"
...
func _initialize() -> void:
	print("=== MIRE A-016a — terrain accents, rock half ===")
