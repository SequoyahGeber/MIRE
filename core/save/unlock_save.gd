...
## the destination, so an interrupted write leaves the PREVIOUS unlock list intact rather than an
## empty file that `load_data()` would silently resolve to "nothing purchased". `false` means the save
## did not happen AND the old one survived.
static func save_data(data: Dictionary, path: String = SAVE_PATH) -> bool:
	data["schema_version"] = SCHEMA_VERSION
	return AtomicJson.write(path, data, "UnlockSave")
