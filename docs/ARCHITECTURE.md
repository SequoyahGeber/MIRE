...
| Wellspring ritual — capped state, channel progress, defense wave (task 4.8) | **Host**. `Wellspring` holds `capped`/`channeling`/`progress_sec`/`duration_sec`/`required_players`; only the host advances or resolves them. | New reliable `net_request_toggle_channel` (client → host, start/cancel); a code-built `SceneReplicationConfig`, all ON_CHANGE, same shape as `Chest.opened` | A client-side ritual timer or presence count is exactly the "two clients disagree" case below — how many players are close enough, and whether the timer has actually elapsed, has to be one answer. The defense wave rides `EnemyWorld`'s existing `MultiplayerSpawner`, so this adds no spawn RPC of its own |
