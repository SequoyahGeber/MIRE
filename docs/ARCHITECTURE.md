...
| Playtesting God mode (F-411) | **Split across existing rows.** The toggle and invulnerability are **Host**; flight is the approved peer's **own player movement** (Client). | The Settings checkbox submits the HOST-scope/op-gated `god` command. `GodModeService` keeps the host's canonical peer set and sends `net_set_local_enabled` only to the approved owner; `PlayerHealth` rejects direct, starvation and Blight damage from the host set, while `PlayerController` alone applies flight velocity. | A client must never grant itself immunity, and the host must never write a remote player's movement. One service state is the extension seam for future requested God-mode abilities. Runtime-only — it is deliberately not persisted as a normal setting. |
...
| Run world seed (task 4.6, F-258/D-161, F-273) | **Host.** `GameState` holds `run_seed`; only the host ever draws one (`host_generate_seed()` at session start, `host_redraw_seed()` at a restart). A client only ever *adopts* a value it was sent, and a seed staged in the menu/`--seed=` affects only sessions that peer itself hosts | `WorldDeltaLog` carries it: `net_world_snapshot` to a peer joining mid-run, `net_delta_applied` (pseudo-chunk `(0,0)`, kind `world`, key `seed`) for a reseed reaching peers already here — no RPC of its own, D-161. Every peer then re-derives locally off the `GameState.seed_ready` signal | Everything derived from the seed (terrain, POI placement, chest/loot streams, Mire layout) is a pure function of it (§4), so one int on the wire replaces all of it. **The seed is per RUN, not per session** — a restart draws a fresh one and re-broadcasts it, so `seed_ready` is a run boundary that fires on every peer, possibly more than once per boundary; that signal's declaration in `core/game_state.gd` states the contract and `tools/seed_ready_contract_check.gd` enforces it |
...
| Chest placement — world-scattered chests (F-146) | **None of its own.** `ChestPlacementService` runs identically on every peer, turning each authored `authored_world_marker` (`kind == "loot"`) into a live `Chest` at the same NodePath everywhere, deriving tier/cost/lock from the marker's own name only — nothing it decides could ever disagree cross-peer. | No RPC. Same "map's deterministic layout builder drops a marker, a bridge instances the live node" split `wellspring_service.gd`/`crafting_service.gd` already use for `authored_world.gd`'s other marker kinds. | `Chest` itself keeps the real authority (host, per `systems/loot/chest.gd`'s own header) — this row only covers WHERE a chest exists, not what happens when one opens. A marker is exactly as safe to build client-locally as the objective/station markers those two services already read from the same group |
...
| Chunk streaming / terrain LOD (task 4.3) | **Client-local**, independently per peer | `ChunkStreamer` streams a ring of chunks around its own anchors; nothing about which chunks are resident is ever sent over the wire | Terrain is never replicated (§4) — every peer regenerates it from the shared seed, so a host and a client streaming different chunk sets around their own local players is correct, not a desync. Only the terrain *content* at a given point must agree cross-peer, and that is `IslandHeightmap`/`BiomeMap`/`ChunkMesher`'s job (all pure and cross-platform-safe), not the streamer's. **`ResourceScatter` (task 4.4) declares this row too**: `placements_for_chunk()` is a pure function of (chunk, seed, authored defs), so WHERE a tree stands never crosses the wire — and because there is no authoritative answer to contradict, a classification disagreement between two generators is invisible at runtime rather than a desync, which is exactly how F-271 survived. The one host-owned thing in the area is a placement's depletion, and that lives in `world/gen/resource_scatter_field.gd` |
...
| Kill bounty — any enemy death (F-539) | **Host.** `Enemy._enter_death()` is reached only through `host_apply_damage()`'s `_owns_simulation()` guard, so `EventBus.emit_enemy_killed()` fires on the host alone; `RewardService` rolls the dead kind's authored `EnemyDef.coin_drop_min`..`coin_drop_max` and grants it to the KILLER's inventory. | No RPC of its own — the grant flows through `InventoryService.host_add()`, already networked. Unlike the row below, this event is deliberately NOT a party fact a client re-derives: there is nothing for a client to do with it, since paying is host-authority anyway. | D-210: paid to the killer, not the party, because a mob is not a party objective and paying everyone would scale the economy with lobby size; and priced as two ints on `EnemyDef` rather than a per-kind `LootTableDef`, since a bounty is one number in one currency |
| Event-granted loot — a Wellspring cap, a boss kill (F-183) | **Host.** `RewardService` rolls the trigger's tier and grants the result straight into every currently-present player's own inventory/powerup stacks; only the host ever rolls or grants. | No RPC of its own — every grant flows through `InventoryService.host_add()`/`PowerupService.host_grant()`, both already networked (each reaches a remote peer through its own existing snapshot RPC). `EventBus.subscribe_wellspring_capped()`/`subscribe_boss_defeated()` already fire identically on every peer (D-107/D-108/F-168/F-181's pattern), so `RewardService`'s own `_owns_mutation()` gate is what keeps a client from rolling a roll nobody would see granted. | D-123: a dynamically-instanced `Chest` at the trigger's position has no established cross-peer NodePath match (unlike `MultiplayerSpawner` or `ChestPlacementService`'s boot-deterministic bridge), so a direct grant through the already-networked inventory/powerup seam needs no new node and no new RPC. One independent roll per present player, not one shared roll — the closer analogue to "whoever gets there first loots" a world chest already means |
| Achievements, stats, rich presence (task 8.3) | **None.** Same shape as Salvage/Unlocks (tasks 6.6/6.9) — per-player account state, no two peers ever compare a stat or an achievement. `SteamStats`/`RichPresenceService` (autoloads) run identically on every peer, each reacting only to events its OWN local `EventBus` received and pushing only to its OWN Steam session. | No RPC. Every trigger rides an existing `EventBus` signal (`run_extracted`/`run_wiped`/`wellspring_capped`/`boss_defeated`/`ship_repaired`/`salvage_banked`/`unlock_purchased`) or a poll of `CycleService.current_cycle()` (F-250: `cycle_advanced` itself still doesn't reach a client, so polling the getter that DOES have the correct per-peer fallback stands in for the broken signal). `SteamStats` persists to `user://steam_stats.json`; `RichPresenceService` calls `SteamLobby.set_status()`. | Same reasoning `ARCHITECTURE.md`'s own Salvage row already gives: this is genuinely private per-account data, "the same category a Steam achievement or a local settings file falls into" — that row named this category before this task existed to fill it. F-249 (fixed) was the one real authority bug this row's own construction found: `ExtractionShip.repair_stage`'s `ship_repaired` emit lived inside a host-only guard, so no client-side consumer of this row could ever have worked before it was moved into the property's own setter. |
