...
**Claim:** `systems/waves/wave_spawner.gd`, `tools/wave_director_check.gd` (+ its `.uid`),
`tools/wave_spawner_cycle_net_check.gd` (new). **Authority:** §2.2 row "Day/night, wave director,
Cycle state, active modifiers: HOST" — unchanged; this task adds no new replicated state and no new
RPC, only a client-side read path onto storage `CycleService`/`WorldDeltaLog` already replicate.
...
`EVENT_BUS.emit_cycle_advanced()` is ever called) gates its whole body behind `_owns_cycle()` —
true for the host or a solo/offline process, false for a real connected client. So a client's local
`EventBus` subscription never fires, and `current_cycle()`'s doc comment claim ("readable on any
peer — same naming convention as `CycleService.current_cycle()`") was false: unlike
`CycleService.current_cycle()`, which earns that claim with a real `_owns_cycle() ? own int :
WorldDeltaLog.latest(...)` fallback, `WaveSpawner.current_cycle()` had no such fallback and just
returned the stale `1` default forever on a client. `cycle_count_multiplier()`'s default arg
(`cycle: int = _current_cycle`) inherited the same stuck value.

**The fix:** `current_cycle()` now takes the identical host-int-or-`WorldDeltaLog`-fallback split
`CycleService.current_cycle()` already uses, keyed on `_owns_wave_director()` — the same
host-or-offline guard every other host-side mutation in this file already checks, not a new
predicate. The `WorldDeltaLog.latest()` call reads the exact `(chunk, kind, key)` address
`CycleService._announce()` writes to, addressed through a `const CYCLE_SERVICE_SCRIPT :=
preload("res://systems/cycle/cycle_service.gd")` rather than a hand-copied literal, so the two
getters cannot drift apart if `CycleService`'s addressing ever changes. `cycle_count_multiplier()`'s
default arg became a `cycle: int = -1` sentinel that reads `current_cycle()` when unset (a GDScript
default parameter value cannot itself be a method call) — so `host_start_wave()`'s bare call now
resolves through the live per-peer path too, and an explicit non-negative `cycle` arg still lets a
check probe any Cycle without waiting for a real advance.

**Verify:** new `tools/wave_spawner_cycle_net_check.gd` — a real two-process ENet host+client, driver
plays the host in-process, a spawned child process is the client (same driver/probe shape as
`tools/day_night_net_check.gd`). The host advances the Cycle for real through
`CycleService.host_advance_cycle()` three times (1 → 4); the client, which never receives the
host-only `EventBus` emission, must still read Cycle 4 back through the `WorldDeltaLog` fallback.
Also asserts the client's private `_current_cycle` cache stays at its `1` default throughout — proof
the getter actually took the fallback path rather than some stray local emission reaching it — and
that `cycle_count_multiplier()` called bare on the client matches the explicit-cycle call.
...
`current_cycle()` via a direct `EVENT_BUS.emit_cycle_advanced()` call, which bypasses `CycleService`
entirely and would have passed even with this bug present — it only proves the getter reads its own
cache, not that a client's cache ever gets updated) stays green:
`WAVE_DIRECTOR_CHECK failures=0`. Regression: `tools/wave_spawner_check.gd`
(`WAVE_SPAWNER_CHECK failures=0`), `tools/cycle_check.gd` (`CYCLE_CHECK failures=0`). Full boot
(`agent godot --quit-after 120`) 0 stray `ERROR:`.

**Swept for the same shape:** grepped every `"readable on any peer"` doc comment project-wide (three
total). `CycleModifierService.active_modifier_ids()` already does the correct
`_owns_modifiers() ? local array : _replicated_active_ids()` split — not buggy. `Wellspring`'s own
`EventBus.subscribe_cycle_advanced()` subscriber (`_on_cycle_advanced()`) is explicitly documented
"Host-only, not replicated" and self-guards on `_owns_mutation()` — it never claims cross-peer
readability, so it is not this bug's shape. No sibling instances found.
...
**Claim:** `systems/environment/day_night.gd`, `tools/day_night_restart_check.gd` (new),
`tools/run_restart_net_check.gd`. Network authority: **HOST**, `ARCHITECTURE.md` §2.2 "Day/night,
wave director, Cycle state, active modifiers" row — unchanged by this task. The host owns
`time_of_day`, advances it, and pushes it to clients over the existing unreliable `net_push_time`;
clients only ever adopt. No new RPC, so no `PROTOCOL_VERSION` bump.

**No spec existed for this finding** — writing it is this task's own first step, per this file's
preamble.

**The shape of the bug.** D-149 enumerated what a run restart resets and did not name the clock, so
`DayNight` initialised `time_of_day` in `_ready()` and never again. `CycleService.host_restart_run()`
zeroed `_days_elapsed` while the clock kept running from wherever the ended run stopped. Runs almost
always end at night, so the next run began mid-night: the day COUNT and the clock disagreed from the
first frame, the new run's first "day" was a partial one, and — the part with teeth — `WaveSpawner`
never got a fresh `night_started` edge, because that crossing had already happened during the
PREVIOUS run. F-259 cleared the latch, but clearing a latch cannot re-fire a signal.

**The fix, in two halves. The second is the one that is easy to miss.**

1. **Host.** `_on_run_restarted()` writes `time_of_day = _run_start_time_of_day` and arms
   `_replicate_elapsed` at a full `REPLICATE_INTERVAL_SEC` so the reset reaches clients on the next
   tick instead of up to a second later. `_run_start_time_of_day` is captured from the export in
   `_ready()`, before anything moved the clock, so retuning the authored default retunes the restart
   with it rather than leaving a second copy of `0.348` to drift. Resetting to morning puts the 0.75
   crossing back IN FRONT of the clock, so the new run's first night arrives the ordinary way.

   **Not through `host_set_time()`** — D-166, and the single thing not to "simplify" here. That seam
   crosses thresholds on purpose; `day_started_at` (0.25) sits between a night-time run end and the
   0.348 morning, so it would fire `day_started` and let `CycleService._on_day_started()` bank a
   phantom elapsed day against the counter `host_restart_run()` zeroed three lines earlier. A run
   BEGINS at morning; nothing crossed into it.

2. **Client.** `net_push_time()` now snaps instead of interpolating when the incoming value is more
   than `CLIENT_SNAP_THRESHOLD` (0.1 of a day) from the last one. A client never runs its own clock —
   it lerps between the last two host snapshots through `_lerp_wrapped_unit()`, which takes the
   SHORTEST way round the wrap, and the shortest way from 0.80 back to 0.348 is backwards through
   dusk and the afternoon the player just played. Snapping the interpolation source from
   `_on_run_restarted()` is **not sufficient on its own**: `net_push_time` is unreliable and
   `run_restarted` reaches a client over `WorldDeltaLog`'s reliable ordered channel, which is
   head-of-line blocked, so the reset push routinely overtakes the event that explains it. Measured
   at 5 smeared frames with the event handler already in place. The threshold is unambiguous — a
   normally-advancing host moves at most `REPLICATE_INTERVAL_SEC / day_length` per push, 0.017 at the
   60-second minimum day and 0.001 at the shipped 900, while the jumps that need snapping (a restart,
   a `time set midnight` from noon) are 0.2–0.5. `core/net/remote_interp.gd` already had this exact
   pattern as `TELEPORT_DISTANCE_M`; DayNight was the outlier. The `run_restarted` handler stays too,
   for the case where the event does win the race — it makes adoption immediate rather than up to an
   interval late.

**Verify:** `.agent/bin/agent godot --script tools/day_night_restart_check.gd` →
`DAY_NIGHT_RESTART_CHECK failures=0`, 23 PASS, exit 0, no undeclared `ERROR:` lines. Two phases, and
phase 2 is a second process for a reason no single process can substitute for:

- **Phase 1 (solo/host, in-process).** Restarts twice from 0.80 through the real path
  (`DefeatService.defeated` then `CycleService.host_restart_run()`) — twice because a reset that
  works once and then latches is exactly the shape F-280 found elsewhere in this feature. Asserts the
  clock returns to 0.348 each time, that NEITHER `day_started` nor `night_started` fired,
  that `days_elapsed_this_cycle()` is still 0, and that advancing the restarted run forward then
  crosses `night_started` exactly once — the WaveSpawner half of the finding, stated as a property.
- **Phase 2 (a real connected client, second process).** The host branch of `_physics_process()`
  never touches the interpolation fields, so nothing in one process can observe a smear at all. The
  probe joins, adopts the ended run's night, and then samples its own `time_of_day` **every frame**
  across the restart, counting any frame inside `(0.42, 0.78)` — a band its legitimate states (0.348
  default below, 0.80 tracked above, and a first snapshot that snaps rather than lerps) cannot reach
  by any route but interpolating backwards. 0 such frames, ending at 0.3493, still firing no
  threshold signals. Its result file is read through a last-known-good cache so a mid-write parse is
  one stale sample rather than a spurious failure (F-290).

**Pre-fix proof.** `agent baseline` cannot help — the check script does not exist at HEAD, the same
constraint F-261 and F-275 both hit. Instead the subscription and the snap were temporarily removed
from the working copy (a copy kept aside; deliberately not `git stash`, which would touch other lanes'
uncommitted work) and the same check run: **`failures=5`**, and they are exactly the finding's
symptoms — the clock stays at 0.800 after both restarts, and advancing the "new" run forward crosses
`night_started` **zero** times. Restored, same check, `failures=0`.

**One check was fixed, not just added.** `tools/run_restart_net_check.gd:171` — the assertion that
REPORTED this finding — used `is_equal_approx(time_of_day, 0.348)` two awaited frames after the
restart, on a clock that is running. A 900-second day advances ~1.9e-5 per physics tick, twice
`CMP_EPSILON`, so it could never have passed even against a correct fix. Loosened to a 0.001 band
(still 450x tighter than the ~0.45 error this finding is about) with the observed value printed. It
now reports the clock PASS at 0.34806; its remaining 4 failures are F-277 (2), F-279 and F-280.

**Swept for the same shape.** Three greps, `--include=*.gd`. (a) Every other client-side interpolator
over replicated state: only `core/net/remote_interp.gd`, which already snaps and needs nothing.
(b) Every `_check_thresholds`/`_crossed` call site: both in this file. (c) Every `"unreliable"` RPC
repo-wide: two, this one and `player_health.gd`'s advisory stamina report, which is not interpolated
— but reading it surfaced a real sibling of the class, filed as **F-298**:
...
run. Left to F-279's owner, who already holds that file's other restart defect.
...
**Claim:** `autoload/attunement_service.gd`, `ui/attunement/attunement_ui.gd`,
`tools/attunement_restart_check.gd` (new), plus the four docs. Network authority: **HOST**,
`ARCHITECTURE.md` §2.2 "Attunement selection" row — unchanged by this task. Only the host records,
clears or grants; a client only ever requests, and its mirror follows the existing
`net_attunement_selected` broadcast. `attunement_ui.gd` declares "none — client-local presentation",
also unchanged. **No new RPC, so no `PROTOCOL_VERSION` bump.**

**No spec existed for this finding** — writing it is this task's own first step, per this file's
preamble.

**The shape of the bug.** D-071 ties an Attunement pick to "run start" and D-010 says a run is one
sitting, so a selection is RUN-scoped state — exactly like the `PowerupService` stack it grants.
F-243 taught `PowerupService` that (`host_clear_all()` on `run_restarted`) and left
`AttunementService` behind: it had no `run_restarted` subscription at all, so run two began with the
Powerup gone and `_process_selection()` still refusing every new pick as "already selected". The UI
half was worse: `AttunementUI` stops its D-071 poll timer permanently after the first pick and had no
restart path, so the mandatory picker never reopened. The player got a run with no Attunement and no
way to choose one.

**The fix,** two files, each on its own side of the authority line:

1. **`autoload/attunement_service.gd`** subscribes `EventBus.run_restarted` and calls a new
   `host_clear_all() -> int`, built on a new per-peer `host_clear_selection(peer_id) -> bool`. The
   per-peer seam is the erase + `net_attunement_selected(peer, &"")` broadcast +
   `selection_changed` emit that `_on_run_player_expired()` already open-coded, which now just calls
   it. Reusing it per peer rather than wiping `_selections` in one go is what gives every peer its
   own broadcast and its own `selection_changed` — and that signal is what re-arms each peer's
   picker. The handler is unconditional on authority and `host_clear_all()` self-guards, the house
   shape every `run_restarted` subscriber in this repo uses, so a client's own copy of the event is
   a no-op and its mirror is updated by the host's broadcast instead. Both return counts, so a
   restart with nothing picked reports 0 rather than claiming work it did not do.
2. **`ui/attunement/attunement_ui.gd`** re-arms the D-071 trigger on a run boundary, driven from
   **both** `EventBus.run_restarted` and `AttunementService.selection_changed`. Both, not either:
   on a client the host's clearing broadcast and the re-derived `run_restarted` ride different
   channels and can land in either order, and whichever arrives second is a no-op because
   `_rearm_for_new_run()` returns early unless the LOCAL selection really is empty. The re-arm polls
   through `call_deferred`, **not** immediately — `run_restarted` subscribers run synchronously in
   autoload registration order and `AttunementUI` is registered ahead of `DefeatHud`/`ExtractionHud`,
   each of which restores `Input.mouse_mode` to `CAPTURED` inside its own handler. Opening during
   that emit would sample the terminal overlay's VISIBLE cursor as "what to restore afterwards" and
   then get the mouse captured out from under a panel whose only mouse control is a CHOOSE button.

**F-297 is fixed here too**, because it is the same file's other defect and its own text names F-277's
owner. `_picking` is now a bounded wait rather than a latch: `choose()` arms a one-shot
`REQUEST_TIMEOUT_SEC = 8.0` Timer, any `selection_confirmed` disarms it, and an expiry re-enables
every CHOOSE button and puts "No answer from the host — pick again." in the existing error status
line. Without it, a host that drops between the request and the answer left a mandatory panel — no
Esc, no dismiss, in `blocks_gameplay_input` — with zero operable controls and no way back to
gameplay.

Public seams added for the check, in the same style as the file's existing `poll_now()`:
`is_picking()`, `operable_button_count()`, `pending_request_seconds_left()`,
`expire_pending_request_now()`.

**Verify:** `.agent/bin/agent godot --script tools/attunement_restart_check.gd` →
`ATTUNEMENT_RESTART_CHECK failures=0`, 49 PASS, exit 0. Two phases:

- **Phase 1 (solo, shipped `levels/hollowmere.tscn`).** Opens the real picker, picks `forager`,
  asserts the pick locked *within* the run (F-277 is about the lock outliving the run, not about
  removing it), then drives the real ending → restart path — `DefeatService.defeated` then
  `CycleService.host_restart_run()`, never a bare `EVENT_BUS.emit_run_restarted()` shortcut (F-291)
  — and asserts run two is genuinely pickable: selection cleared, backing Powerup gone, picker
  reopened with all four CHOOSE buttons operable and one of them holding focus (F-216's no-mouse
  guarantee has to survive into run two too), no stale `_picking`, and a **different** role
  accepted. Idempotence is asserted, because the handler runs on every restart forever.
- **Phase 2 (a real connected client, second process).** The fix's host-clears/each-peer-re-arms
  split is exactly what one process cannot see. A really joined client picks over the wire, the host
  restarts, and the client reports whether ITS selection cleared and ITS picker reopened; it then
  picks a second, different role over the wire, which is what proves the host's lock lifted for a
  *remote* peer and not just locally. A third restart proves the re-arm is not one-shot. The client
  also owns the F-297 sub-phase, in the finding's own scenario rather than a simulated one: it sends
  a request and then really `leave()`s the session so the answer can never arrive, asserts the
  bounded wait is armed (`pending_request_seconds_left() > 0`), and only then runs it out through
  the debug seam rather than stalling the check for 8 seconds. The client also asserts its own
  `host_clear_all()` returns 0 — a client must never clear a selection itself.

**Pre-fix proof.** `agent baseline` cannot help (the check does not exist at HEAD, the F-261/F-275
...
check run against them: `failures=18`, and they are exactly the finding's symptoms on both sides of
the wire (selection still `forager` after the restart, picker never reopens, no focus, run two
refuses a different role, the client's mirror and picker likewise, and no bounded wait at all).
Files restored, same check, `failures=0`. The check's calls into seams the fix *adds* are wrapped in
`has_method` guards for this reason — a harness that aborts its own coroutine on the first missing
method cannot produce a pre-fix proof.

**Neighbours re-run green at this commit:** `run_restart_check` 0, `terminal_focus_check` 0,
`attunement_check` 0, `attunement_ui_check` 0, `attunement_net_check` 0, `powerup_check` 0, each with
0 `ERROR:` lines; generic `agent godot --quit-after 120` exit 0 with 0 `ERROR:` lines. The
`attunement_restart_check` run itself emits 4 engine `ERROR: Condition "!_has_authority(spawner)"`
lines from the client process — pre-existing and already filed as **F-296**, not caused here.

**Swept for the same shape.** Three greps. (a) Every `"already …"` refusal across `autoload/`,
`systems/` and `core/`: `AttunementService`'s was the only permanent, run-scoped, one-pick lock —
`NetTransport`/`SteamLobby`'s are session-state guards and `WaveSpawner`'s is transient. (b) Every
`ui/` file carrying a `_pending`/`_awaiting`/`_requested` in-flight latch: `chest_ui.gd` and
`crafting_ui.gd` both have `ui_cancel` dismiss paths, so an unanswered request there is recoverable
and not F-297's class; the only mandatory panels are the two terminal overlays F-275 fixed and this
one. (c) Every autoload against `subscribe_run_restarted`: the remaining run-scoped gaps are already
enumerated as F-281 and being worked by other lanes (F-278 DayNight, F-279 spawn, F-280
