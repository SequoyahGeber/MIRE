...
| A-000T | `DONE` | **Tree trunk rebuild, at Sequoyah's direct request and out of queue order** — A-016b stays `NEXT` and unstarted, the same precedent as A-004R, A-021S and A-000V. F-396 had already replaced the kit's two-cylinder trunks with a tapered stack plus a root flare, bark ridges and buttress roots, and Sequoyah called them out again anyway. F-422 is why: the defects belong to the STACK, not to its parts. Rebuilt `tree_trunk()` on a new `tube_mesh()` primitive — one continuous flat-shaded tube through vertex rings, with radius, material and shape varying per ring and per column. The flare and its buttress lobes are the tube's own bottom rings; the bark grain is flutes in the same surface; the root toes are extra reach on the sharpest lobes over the first ~0.3 m; birch lenticels and crooked-tree moss are faces of the tube painted in a second material. Nothing is butted onto anything, so the stepped silhouette, the splayed root 'chicken foot', the birch's dark boot and the black ridge-stick beside the trunk all have no mechanism left. Rebuilt all 18 kit trees (4 species) plus the other 110 kit assets unchanged. Triangles went **down** — pine 63 loose cones to 1,036 tris in one mesh — because a tube has no internal end caps. New instrument: `tools/blender/trunk_inspect.py`, ten views per asset (eight azimuths, top with the crown clipped away, bottom) framed on the bottom 4.2 m, because `audit_all_sides` renders a 19 m tree with its trunk four pixels wide and could never have shown any of this. Verified: `audit_all_sides` numeric report **0 defects across all 18** (no inside-out, no open surfaces, no degenerate faces, no loose verts, no unapplied transforms); trunk contact sheets inspected from all ten views for every species; `tools/tree_collider_check.gd` extended to cover the four kit species and run — 0.50-0.61 m collider radius, **0.00 m worst overhang**, and the pre-existing script error on colliderless props fixed so it now finishes its own subject list | 18 rebuilt | F-422 |
| A-000W | `DONE` | **Willow rebuilt from the real species, at Sequoyah's direct request and out of queue order** — A-016b stays `NEXT` and unstarted. F-424: `tree_willow` was the flora kit's default slim trunk plus its default round canopy, with six fat teardrops on the rim — a generic tree wearing a species name. Rebuilt to `Salix babylonica`'s actual proportions: a short **very thick** deeply-furrowed bole (0.78 m base radius, forking at 24% of height instead of 62%), three or four heavy ascending limbs from that low fork, branches that arch over and start back down, and ~80 slender tapering whips falling nearly to the ground in the pale willow greens (`leaf`/`leaf_light`/`leaf_pale`) rather than forest green. No capping canopy hull at all — a willow's crown surface IS its hanging strands. 2,254 tris against the family's 2,400 cap. Two general fixes fell out: `mire_art.hull` gained **`taper_low`** (narrow the BOTTOM — `taper` narrows the top and a negative `taper` flares it, which the camp and food sets already depend on), and the willow's authored height was raised to fill its size band because `create_asset`'s uniform scale-to-band was stretching a 10.3 m tree to 13.6 m and taking its root flare through `COLLIDER_TRUNK_BAND_MIN_M` with it — F-390's exact defect, collider 1.32 m around a sub-1.0 m bole. Also in this batch: F-422's `trunk_tube` moved into `mire_art` and **both** kits now build on it, so the flora snag and willow lose the same stepped silhouette and splayed root cones the map kit's trees did — the map kit rebuilds byte-for-byte identical across all 128 assets, proving the move was a pure refactor. Verified: flora build contract PASS 84/84 (size band, footprint cap, colour cap, ground contact, no floating mesh islands, triangle budget); ten-view trunk sheet inspected from all eight azimuths plus top and bottom; `tools/tree_collider_check.gd` 1.02 m willow collider, 0.00 m worst overhang | 84 rebuilt | F-424 |
| A-000H | `DONE` | **Five-state harvest chains and a 15.4 m harvest tree, at Sequoyah's direct request and out of queue order** — A-016b stays `NEXT` and unstarted. F-425. Every harvestable now runs **intact + three partials + depleted** instead of two or three states across six to nine swings, and each stage is what the real job does to the material: the tree opens a face cut, drives the notch most of the way in while shedding branches into a growing chip pile, then reaches a nearly-severed hinge where **it visibly leans into its own notch** (bend above the cut, never a tilt of the whole prop, so the shared footprint holds); the stone and iron nodes go `chipped` (pale fresh-fracture scars, nothing broken off yet), `cracked`, `shattered` (top mass gone, jagged stub in its own rubble skirt), with iron's seams widening as it comes apart so the closer it is to spent the more ore shows. 12 exports to **17**. Separately and more seriously, the **harvest tree was 5.35 m** — F-396 raised every tree in the environment and flora kits and missed this one because it lives in the harvestable kit, so the one tree the player stands under was the shortest in the game at 3.1x eye height. Now **15.4 m** with the height authored in, not scaled (base radius 0.48 -> 0.62 against height 5.35 -> 15.4), six branch tiers starting at 6.6 m, the axe notch left at 0.80-1.48 m where a person can actually swing, and the stump and felled trunk grown to match the trunk they came from. New instrument `tools/harvest_state_chain_check.gd`: adding a state does not guarantee anyone ever sees it, so it walks each definition's health from full to 1 and asserts every index is visited, that damage never visibly reverses, and that every named scene loads. Verified: state-chain check **0 failures** (3 chains x 4 states each, 10 authored-visual definitions correctly skipped), `harvestable_check` 0 failures, `harvest_world_check` 0 failures, `harvest_batch_check` 0 failures, both previews re-rendered and inspected | 17 exports | F-425 |
| A-050 | `DONE` | **Nothing in the game is missing art, at Sequoyah's direct request and out of queue order** — A-016b stays `NEXT` and unstarted. New instrument first: `tools/art_coverage_check.gd` loads every `.tres` under `content/`, derives the art slots from the definition scripts' **own exported properties** (so a new art field shows up the first time anyone runs it rather than being silently uncovered), and separates MISSING from BROKEN. First run: **211 slots, 100 missing**. Closed all 100. (1) **72 powerup icons + 4 attunement sigils** (F-426 is separate; see F-425's sibling) — modelled, not drawn, because a powerup has no prop to point a camera at and 72 sprites would read as a different game beside an item icon in the same window. A tag-family plaque carrying the family in shape AND colour, plus a per-powerup motif that says the EFFECT rather than illustrating the name. `fit_to_plaque()` measures and shrinks each motif, and `tools/powerup_icon_check.gd` guards the tag duplication the Blender script cannot avoid. (2) **The wooden wall** — the first thing most players build, and the one buildable with no scene at all, so every wall in the game was `BuildGhost`'s generated grey box. Board-and-batten on posts, mixed board widths, posts defining the mating plane. (3) **13 build-menu icons**, rendered from the same GLB each piece places and forced upright against the renderer's packing heuristic — architecture has a known up. (4) **9 raw-material viewmodels**: log, branch, fibre, stone, flint, coal, ore, ingot and coins are now held rather than shown as an empty hand, each with its own authored grip; `viewmodel_check` 0 failures after pulling the coin grip off the camera near plane. (5) **The heavy ore crate** — the last empty slot, and a prop two players walk a long way behind: banded timber on vertical staves, corner irons, rope beckets at both ends (which is the read for *needs two of you*), and a load standing proud of the rim. Verified: **ART_COVERAGE_CHECK slots=211 missing=0 broken=0**, `powerup_icon_check` 76/0, `viewmodel_check` 0, `harvest_state_chain_check` 0, `harvestable_check` 0, `tree_collider_check` 0.00 m overhang, camp and construction build contracts PASS, all-sides sheets inspected for both new props. F-428 filed for `item_icons_check`, which is red at HEAD on 78 untracked stale duplicates that predate this batch | 89 icons + 2 props | F-427 |
| A-051 | `DONE` | **Food: berries that show their fruit, an apple tree, a mushroom patch, at Sequoyah's direct request and out of queue order** — A-016b stays `NEXT`. F-441. The berry bushes existed and did not work: the fruiting and picked states were indistinguishable because `berry_sites()` hung the clusters UNDER the foliage masses with the elevation biased downward, and the berries were 7-9 cm across (the size figure was used as a radius) — two errors that cancelled into "you can sort of see some red". Rebuilt on **arching canes**, which is what a bramble is: the fruit ends up outside the leaves for free, because that is where the plant puts it. Foliage masses shrank hard — at their old radii they engulfed every cane. New **apple tree** state pair, built as a `Malus domestica`: short thick trunk forking low into five uneven scaffold limbs that each kink once (apple wood turns, it does not sweep), a crown as wide as the tree is tall, fruit on the crown's OUTER surface where a spur bears, and every apple two-toned — a blush and a green-gold cheek, because one flat red at that size is a cherry. New **mushroom patch** state pair in new `fungus_edible`/`fungus_gill` tokens: the kit's six toadstools are decoration in the mire's growth-signal pink and blue, which a mushroom the player EATS cannot wear; `pickup_mushroom` was recoloured off `mire_flesh`/`mire_light` for the same reason. New `pickup_apple` (two apples and a twig — the kit's legible-by-quantity rule) and `icon_apple`. Wired: `content/items/apple.tres` plus `berry_bush`, `apple_tree` and `mushroom_patch` harvestable defs — the berry bush GLBs had been shipped with **no definition referencing them at all**. Also fixed F-440 on the way through: five pickups had been 16-24% under their declared size since `world_bounds` stopped using the inflated `bound_box` ruler, so the pickup build was red at HEAD; `create_asset` now normalises onto `SCALE` the way every other kit does. And `item_icons_check` now REPORTS strays instead of failing on them, so it is readable again. Verified: gatherables contract PASS 12/12 with berry-bush state drift **0.000 mm**, pickup scale contract PASS 15/15, `art_coverage_check` **220 slots / 0 missing / 0 broken**, `harvest_state_chain_check` 0, `harvestable_check` 0, `item_icons_check` PASS, ten-view sheets inspected for the apple tree and the mushroom patch | 5 new + 4 rebuilt | F-441 |
...
| A-015 | `DONE` | **Six of the twelve listed** — swamp willow, lily pads, marsh grass, sedge, bog flowers and water reeds already ship in `assets/flora` (`tree_willow_a`..`c`, `lily_pad_a`..`c`, `marsh_grass_a`..`c`, `sedge_a`..`c`, `flowers_bog_a`..`c`) and `assets/environment` (`reeds_a`..`d`), and remaking them would put two objects in the world competing to be the same plant. Made 6 in a new `wetland_nature` kit (`assets/wetland_nature/`, `tools/blender/build_wetland_nature.py`): alder, hollow tree, uprooted tree, mangrove tree, duckweed, hanging moss. **3,936 triangles, 512–960 per asset**, at most 4 materials each, Blender 5.2.0 LTS. Each tree is built around one silhouette the existing seven cannot make (multi-stem stool, a void through the trunk, a vertical root plate, a trunk standing clear of the ground) — a seventh trunk-with-canopy is scenery nobody notices. **Three lessons worth carrying:** a crown is the one place A-000V's "three masses, never eighteen" does NOT apply — three on a branching tree read as parasols on sticks, and seventeen are affordable only because canopy masses are **base icosahedra** (`subdivisions=0`, 20 faces against 80), which took the alder from 1,580 to 960 triangles while looking better and matters because there is still no prop LOD (F-144); a mouth is only a mouth if the rest of the wall is solid (the hollow tree's staves first stood clear of each other and the trunk read as a bundle of poles, so the missing ones said nothing); and roots branch rather than radiate — eleven straight rods out of the root plate rendered as a sea urchin until they were rebuilt with `fork()`. Verified: build contract **fails** rather than warns on 7 conditions, 6/6 pass; all-sides audit against a clean `--outdir` (F-110) with **0 numeric defects**; `tools/wetland_nature_check.gd` **25 assertions, 0 failures**, measuring every import from its vertices rather than `Transform3D * AABB` (F-108). Known rough: the mangrove's trunk is spindly between stilts and crown, and the root plate is smoother than torn soil should be — both read at placement distance, neither has had a second art pass | 6 exports | A-000 |
| A-016a | `DONE` | **A-016 split into lettered children** per this file's own rule for a batch too large for one clean commit. This is the rock half: cliff face, cliff corner, cliff overhang, rocky slope, scree pile, natural stone steps. **The gate was met, not waived** — A-016 waited on "world-gen terrain shape is settled" and **D-142 settled it 2026-08-19**. Made 6 in a new `terrain_accents` kit (`assets/terrain_accents/`, `tools/blender/build_terrain_accents.py`); **3,184 triangles, 280–920 per asset**, at most 3 materials each, Blender 5.2.0 LTS. **`cave entrance` was NOT built — filed as F-237**: D-142 rejects 3D density and caves outright ("2D Mire grid, low-end target, cut list"), a 2D heightfield cannot express an overhung void, and a prop that reads as an entrance backed by solid ground is a bug report from every playtester who finds one. **`cliff_face` and `cliff_corner` share one 4.00 m module width and 5.00 m height with flat vertical backs at y = 0, asserted by the build** — and bed width is clamped never to exceed the module, because jitter that could exceed 1.0 made a "4.00 m module" 4.48 m and two placed side by side pushed beds through each other. **Four lessons:** boxes make masonry (the first cut read as concrete blocks — `stone_wall_solid` already exists and a cliff that looks built is worse than no cliff), anything whose read is its profile must present that profile to the camera (three assets were authored extending along +Y and rendered end-on), a `(depth, width)` tuple unpacked backwards rotates a module 90°, and a negative base to a fractional power is complex rather than small. **An unused material slot is a lie in the catalog** — `paint_faces` left a moss slot with zero faces on the overhang, Blender counted it and glTF did not; the generator now counts only materials a polygon references. Verified: build contract **fails** rather than warns on 8 conditions, 6/6 pass; all-sides audit against a clean `--outdir` (F-110) with **0 numeric defects**; `tools/terrain_accents_check.gd` **24 assertions, 0 failures**. Known rough: `cliff_corner` is thick through the corner and has had one pass, not two | 6 exports | D-142 |
| A-016b | `NEXT` | Terrain accents, the earth-and-water half: mud bank, riverbank, streambed, sinkhole, **burrow mound**. `sinkhole` needs re-scoping to a shallow surface depression for F-237's reason — a 2D heightfield has nothing for a hole to go into. **`burrow entrance` re-scoped to `burrow mound` by F-255's sibling sweep, same reason at a smaller scale**: build a mound of turned earth whose mouth is a shallow, visibly-bottomed hollow in the prop's own mesh, sized for an animal and never for a player — a hole the terrain would have to own is the one thing it cannot own (D-159). `cave entrance` is deliberately absent | 5 | A-016a |
...
| A-020 | `QUEUED` | Landmark kit II: giant hollow tree, crystal grove centerpiece, mushroom grove centerpiece, corrupted crater, Mire nest, **flooded cellar ruin**, broken dam, hilltop beacon. **Re-scoped from `flooded cellar entrance` by F-255, pre-emptively and for F-237's reason** — D-142 rejects caves and 3D density, so the 2D heightfield has no room below grade for a cellar to be, and a doorway leading into solid ground is a bug report from every playtester who tries to walk in. Build it as a **sunken, half-collapsed structure standing at or near grade**: broken foundation walls, a caved-in floor slab, at most a token few steps down into ankle-deep water that visibly bottoms out — never a threshold the eye reads as `go in here`. Any dark recess it has must live inside the prop's own mesh, the way `giant hollow tree`'s void lives inside its trunk (D-159). `corrupted crater` is fine as written: a depression IN the heightfield is exactly what `sinkhole` was re-scoped to | 8 assemblies | A-015, A-017 |
| A-043 | `DONE` | **Built out of queue at Sequoyah's direct request on 2026-08-19** ("start making some assets one by one") while 2.1d was held by another session for the camp and road kits — the same precedent as A-004R/A-021S/A-000V. **Five of the seven listed**: two already existed and were deliberately not remade, `poison_berry_bush` (A-011) and `raw_fish` (A-012's one-fish frame), because a second object competing to be the same item is worse than a gap. Made 5 in a new `wetland` kit (`assets/wetland/`, `tools/blender/build_wetland_set.py`); **4,092 triangles, 680–1,016 per asset**, at most 4 materials each, Blender 5.2.0 LTS. The kit's whole problem is that `assets/flora` already ships `reeds_a`..`d` and `moss_patch_a`..`d`, so each node has to out-read a decorative neighbour standing beside it: the bundle is tied and **lying down** (it is made of the same parts as the bed, so upright the two shared a silhouette and a player could not tell what they can carry from what they must harvest), the bed flowers on only 8 of its stems, the hummock stands up, the shoal shares one heading. **Two palette rules bit and are worth reusing**: the shoal's pool is peat-brown rather than `clear_liquid`, because that renders teal and teal is the Ward's reserved hue — A-014's agent reached the identical conclusion for road puddles independently the same day — and bioluminescence needed a colour nobody had claimed, so `glowcap`/`glowcap_gill` were added append-only in a cold green (purple is the Mire, teal the Ward, `ember`/`flame` fire, `critical` damage, `eye` an enemy). Only the caps are emissive: lighting the gills too turns each mushroom into a glowing blob and throws away the cap-over-stem outline, and `preview/glowcap_night_preview.png` is the sheet that tests it because a sunlit render cannot. Verified: build contract **fails** rather than warns on 8 conditions, 5/5 pass; all-sides audit from eight azimuths plus top and bottom against a clean `--outdir` (F-110) with **0 numeric defects**; `tools/wetland_check.gd` **21 assertions, 0 failures**, measuring every import from its vertices rather than `Transform3D * AABB` (F-108) | 5 exports | A-011 |
