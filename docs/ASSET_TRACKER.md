...
| A-016b | `NEXT` | Terrain accents, the earth-and-water half: mud bank, riverbank, streambed, sinkhole, burrow entrance. `sinkhole` needs re-scoping to a shallow surface depression for F-237's reason — a 2D heightfield has nothing for a hole to go into. `cave entrance` is deliberately absent | 5 | A-016a |
