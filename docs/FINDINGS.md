...
**Resolved 2026-08-19 by lm2.** **fixed** — all seven Cycle Modifiers now have a real gameplay consumer, one per modifier, each
against the exact seam its own `.tres` description already named:

- `drought` -> `Harvestable._yield_amount()` halves the yield; `CycleModifierService.drought_active()`
  tracks the "until the next Wellspring cap" window separately from the permanent draw stack.
- `long_night` -> `DayNight._advance_host()` halves the time-advance rate while in the night phase.
- `tithe` -> `Wellspring._start_channel()` raises `required_players` by one for co-op only (solo
  exempt — see D-156, a naive unconditional +1 bricked solo Wellspring capping).
- `static` -> `Chest._unlock_check()`/`_price_for()` refuses every POWERUP roll and halves `cost_coins`.
- `rooted` -> `MireGrid._tick()` passes an empty ward array regardless of the real provider, so a
  Ward's spread resistance stops applying.
- `bloom` -> `Enemy._enter_death()` spawns two reduced-health children via `EnemyWorld.host_spawn()`.
- `the_hunt` -> `WaveSpawner` spawns one `tusker` (reused content, D-156) per Cycle and retargets it
  every few seconds to whoever leads `PowerupService.total_stacks()` (new), via the new
  `Enemy.host_force_target()`.

Every modifier's `.tres` description was rewritten to drop the stale "no effect is wired" text.

**Verified:** `agent godot --script tools/cycle_modifier_check.gd` (unchanged, `failures=0`) and
`tools/cycle_modifier_seed_check.gd` (unaffected). New `tools/cycle_modifier_effects_check.gd` forces
each modifier active and proves its real effect end to end — `CYCLE_MODIFIER_EFFECTS_CHECK failures=0`,
30 assertions. Full regression sweep across every touched system's checks (harvestable, day_night,
wellspring incl. recorruption/hud, chest incl. seed/placement, mire_grid, mire_interaction, powerup,
enemy incl. ai, bog_crawler, wave_spawner incl. cycle_net) all `failures=0`.

Full account, including the tithe/solo bug caught and fixed mid-task and a GDScript-lambda/EventBus
crash trap found while writing the new check: `docs/SPECS.md`'s F-245 block. Design calls: D-156.
API surface for the next task: `docs/DELEGATION.md` *Current state*.

Left out of scope: the roadmap's remaining un-authored Cycle Modifiers and F-236's sibling gap in the
unlock tree / ranged rack.
...
F-203 closed the emitter half of its own finding but deliberately left sway untouched —
`AuthoredWorld._build_props()` routed anything `AssetVfx.sway_for() != NONE` into the original
per-(chunk, kit, asset) `MultiMesh` path, same as before F-187, because `EnvironmentVfx._apply_sway`
dresses a MESH once from ONE `mesh.get_aabb()` and that AABB stops meaning "one plant's own local
frame" the instant several placements' absolute, chunk-relative heights are baked into one static
mesh — the mask would then read terrain elevation across the merge instead of height within each
individual plant. Full mechanism: `docs/SPECS.md` F-203 block, "Scope decided here" note.

**Resolved 2026-08-19 by lm.** Built option 1 from the finding's own list — a per-vertex baked
height-mask, the harder of the two mechanisms F-203's own title named:

1. `core/render/mesh_merge.gd`'s `merge_instances()` gained a `bake_height_mask` parameter. When
   true, every output vertex's UV2.x is overwritten with that vertex's normalised height computed
   from ITS OWN entry's mesh's local AABB (`mesh.get_aabb()`, the exact bounds `_apply_sway` would
   have used un-merged) — BEFORE `transform` moves it into the merged holder's shared space. Two
   different source assets in one merge bucket keep two different, individually-correct masks.
2. `world/environment/foliage_wind.gdshader` gained a `use_baked_mask` uniform: when true, the
   vertex shader's height mask comes from `UV2.x` instead of computing
   `(VERTEX.y - wind_root_y) * wind_inv_height`. Default false, so every existing per-asset
   `MultiMesh`/loose-mesh sway material (the vast majority of swaying instances) is byte-for-byte
   unchanged — this is additive, not a rewrite of the existing path.
3. `world/gen/authored_world.gd`'s `_build_props()` gained a third merge bucket, `sway_mergeable`,
   keyed `"<chunk>|s<sway_int>"` — the same shape F-203 used for `emitter_mergeable` keyed by
   emitter class, and folded into the shared merge/bake/`DrawPolicy` loop the same way. Eligibility:
   non-harvestable, sub-`SHADOW_MIN_HEIGHT` (unchanged guard, so the F-203 shadow-cascade trap still
   cannot recur), sway-bearing, and carrying NO emitter. **Scope decision, recorded here rather than
   a new D-number:** an asset carrying BOTH sway and an emitter (`mire_tendril`: TENDRIL + SPORE is
   the one on Hollowmere) stays excluded from every merge bucket, same as it always was — merging it
   into the sway bucket would need `EMITTER_META` and a baked mask on the same holder at once, and
   the emitter bucket would silently drop its sway. Not a regression: F-187 already excluded every
   sway-bearing asset from any merge, so this only narrows what F-208 itself newly enables.
4. `autoload/environment_vfx.gd` gained `SWAY_META` (`&"vfx_sway"`, an `AssetVfxLibrary.Sway` int)
   and `_merged_sway_for()` — the `EMITTER_META`/`_merged_emitter_for()` shape applied to sway —
   checked in `_apply_node()` before the asset-id walk. A merged sway holder routes to the new
   `_apply_baked_sway()`/`_baked_sway_material()`, which dress the mesh with the sway PROFILE only
   (no per-mesh AABB, since the mask is already baked) and set `use_baked_mask = true`.

**On Hollowmere specifically:** `merged_meshes` 28 → 67 (39 new sway-class buckets),
`multimeshes` unchanged at the shape level though the live count dropped as expected (some grass/
frond/flower instances left their per-asset `MultiMesh` for a merged holder), `merged_sway_instance_
count=456` (individual placements now baked into a sway merge). Draw calls (`frame_cost_check.gd`
"as shipped" vs `agent baseline`, HEAD pre-fix): 4,936 → 4,864 (a further win, not just parity).
Primitives: 1,147,078 → 1,171,296 (+2.1%, the same kind of small LOD-boundary shift F-203 saw
splitting its own buckets, nowhere near the +16–18% shadow-cascade regression F-187/F-203's history
warns about). VRAM: 253.2 → 278.2 MB (+9.9%, the real, expected cost of the new UV2 channel plus 39
more merge buckets — traded for the draw-call win). `frame_ms` "as shipped" swung 13.70 → 17.86 in
one run and 13.70 → 9.43 in a second back-to-back run on the SAME built code — confirmed as run-to-
run scheduler noise from other agent lanes sharing this machine, not a regression: every `preset`
row (high/medium/low, which don't share "as shipped"'s sampling window) stayed flat or improved
across both runs.

Verified:
- `agent godot --script tools/mesh_merge_check.gd` — `MESH_MERGE_CHECK_GODOT PASS`, including a new
  `_check_merge_instances_baked_height_mask()` synthetic test: two boxes at different world
  offsets, each baked from their own local AABB, prove the mask ignores a placement's translation
  (every UV2.x lands in `[0, 1]`, both a ~0 root vertex and a ~1 tip vertex are seen).
- `agent godot --script tools/prop_chunk_merge_check.gd` — `PROP_CHUNK_MERGE_CHECK PASS`,
  `eligible_props=719 eligible_chunks=67`, matching the 67 `merged_*` holders built — its own
  independent eligibility recompute updated to bucket by `(chunk, sway type)` the same way
  `_build_props` now does, excluding a sway+emitter combo the same way. Every merged holder's own
  `cast_shadow` still reads OFF — the F-203 cascade-regression invariant, unaffected by this fix.
- `agent godot --script tools/environment_vfx_hollowmere_check.gd` — all existing assertions PASS,
  plus a new `merged_sway_instances` reading (from a new `AuthoredWorld.merged_sway_instance_count`
  stat, since a sway holder publishes no per-instance `placements` for anything else to count from)
  folded into the `swaying_copies > 1000` coverage assertion, plus a new
  `merged_sway_instances > 0` assertion proving the new bucket actually engaged on this map.
- `agent godot --script tools/hollowmere_check.gd`, `tools/harvest_batch_check.gd`,
  `tools/harvest_world_check.gd`, `tools/resource_scatter_check.gd` — all PASS, unaffected.
- No shader/material errors in any of the above runs (checked stderr directly).

Full writeup: `docs/SPECS.md` F-208 block.
...
**Resolved 2026-08-19 by lm.** Doc-only fix. ARCHITECTURE.md:203-204 replaced the stale "PackedByteArray RPC" replication bullet
with the real, permanent mechanism task 4.9 shipped: WorldDeltaLog.host_record(chunk, kind, key,
value) per changed cell (task 4.6's log), matching D-099's own text and mire_grid.gd's header
comment. No code changed - MireGrid already implemented this correctly; only the doc was stale.
Verified by re-reading docs/ARCHITECTURE.md §5 (no PackedByteArray mention remains) against
world/mire/mire_grid.gd:3-19 and docs/DECISIONS.md D-099. Wrote the missing SPECS.md F-212 block
per this file's own preamble ("if a spec is missing, fixing it here is part of the task that
discovers it").
...
**Resolved 2026-08-19 by lm.** Added one Marker3D (kind "shipwreck") to world/gen/layouts/hollowmere.json at MereShore [62.0, 1.54,
29.0] — the exact spot the layout's own "extraction pad/cache/ward/rail/markers" props already
built a dock around with nothing ever placed on it. autoload/extraction_service.gd picked it up with
no code change, same recipe wellspring_service.gd's "objective" marker proves. Also fixed
tools/hollowmere_check.gd's _probe_ground, which had a latent gap this exposed for the first time:
it only skipped authored_world_prop colliders, so the new live ExtractionShip's runtime-built
StaticBody3D (same shape as Wellspring's own) read as a 3.9m ground-collision failure; now skips
colliders parented to &"wellspring" or &"extraction_ship" too, and added a
HOLLOWMERE_SHIPWRECK check that proves the marker->live-ship bridge fires against the real map.
Verified: agent godot --script tools/hollowmere_check.gd -> HOLLOWMERE_MARKERS includes
"shipwreck":1, HOLLOWMERE_SHIPWRECK marker=Shipwreck ship_built=true, HOLLOWMERE_GROUND
worst_delta=0.000m, HOLLOWMERE_CHECK PASS. agent baseline --script tools/hollowmere_check.gd
confirms HEAD already passed (not a check this task turned from red to green, the marker just gave
the ground-probe gap something to hit for the first time). tools/extraction_check.gd still
failures=0. Full spec written at docs/SPECS.md under F-166.
...
**Resolved 2026-08-19 by lp.** Fixed. F-200 built tools/autoload_tracked_check.py (mechanism #1: verifies every project.godot
[autoload] target, and everything it transitively preload()s, is a git blob tracked at a given
revision) but nothing called it pre-commit. cmd_check now does: a new _autoload_tracked_missing()
in .agent/bin/agent imports autoload_tracked_check and calls sweep("") whenever the STAGED/working
change set includes project.godot or any .gd file. Empty-string --rev was already free in the F-200
script — its tracked_at()/read_at() build "%s:%s" % (rev, path), and an empty rev collapses to
git's own ":path" INDEX syntax, exactly the "staged content if any, HEAD's otherwise" overlay a
pre-commit hook needs — so no new revision-diffing logic and no change to autoload_tracked_check.py's
checking code (only its docstring, documenting --rev ""). Anything sweep("") reports missing becomes
a cmd_check error, blocking the commit, independent of the claim loop. A checker import/sweep
exception fails OPEN (never blocks a commit over the checker's own bug — F-200's after-the-fact
check still catches that separately).

Verified:
- python3 tools/autoload_tracked_check.py --self-test -> 4/4 passed (added
  catches_staged_precommit_f205_shape: stages an autoload registration, never `git add`s its target,
  checks --rev "" with no commit to name by sha yet -- proves the INDEX view, not --rev HEAD, is
  what a real pre-commit hook needs).
- python3 tools/harness_check.py -> 34/34 passed. Three new cases drive cmd_check through a
  simulated git hook (run(..., as_hook=True) sets GIT_INDEX_FILE so cmd_check takes the
  STAGED/INDEX branch, F-001): an autoload registered with its script staged nowhere (blocked,
  names the path, cites F-200/F-205); a staged autoload script whose own preload() target is
  untracked (blocked, names the transitive path); and the sanctioned path -- a new autoload's
  script staged in the SAME commit (passes silently, only F-051's existing warning -- no
  false-positive on the one case this must never block).
...
- .agent/bin/agent check on the real working tree -> clean pass (this task's own files don't
  touch project.godot/.gd in a way the new gate should catch).
- agent godot --script tools/findings_numbering_check.gd -> confirms FINDINGS.md heading integrity
  after this move (F-134's trap).

Swept for the same shape elsewhere: grep -rln "cat-file -e\|tracked_at\|res://" tools/*.py finds
only autoload_tracked_check.py (and now harness_check.py, which imports it for the new test cases)
doing git-blob-tracked-at-rev checking. The repo's other tools/*_check.py files
(godot_prepass_check.py, import_cache_guard_check.py, png_pixels_equal_check.py) verify unrelated
mechanisms and are already wired into what calls them -- no other "checker built, nothing calls it
pre-commit" instance found.

Full spec: docs/SPECS.md F-205.
